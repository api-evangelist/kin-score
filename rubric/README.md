...
| Version | File | Notes |
|---------|------|-------|
| 0.13.0 | [`scoring-0.13.0.yml`](scoring-0.13.0.yml) | **The OIDC discovery document, finally read.** `auth_clarity` gains a `served` tier at 0.90 — a served `openid-configuration` carrying `issuer`, which the pipeline had been probing on 5,881 providers and saving on 889 while no check read it as OIDC. Tests the required field, not the media type. Adds `delegated_identity` (6 pts, slot 16), which raises max 117 -> 123 and rescales every agent score by 0.9512. Batched with 0.12.2 so all of it shares one band re-cut. |
