...
0.12 introduced is over, four releases before the 1.0 boundary it was scheduled for.

It came forward because the migration aid had become a defect. `/rating/facets` served **eight
names for six facets** — `access_clarity` and `commercial_clarity` carrying the same number, the
same for `contract_governance` and `governance` — so a consumer summing the object double-counted
two of nine, and the published `FacetName` enum listed neither alias, so the contract gave no way
to find out.

The 0.12 note priced a hard cut at ~27,300 files including "669 report bundle data files across 118
SOLD reports". Measured today, that is 15 files across 4 repos: the reports were retired on
2026-08-21 and answer 410, the 26,574 provider pages are regenerated by the scorer, and the badge
renderer is one file rather than 38.

No score moved. The old names stay accepted as INPUT and are no longer returned.

Next in this family:

- **The `kin/score` artifacts still carry both spellings** for every pass written before today.
  They accumulate per run rather than being rewritten, so old artifacts keep old keys by design —
  worth stating in the artifact's own documentation rather than leaving a reader to infer it.
- **1.0 loses one of its stated jobs.** The rename was the cleanup that "rides a boundary that was
  already going to be a hard cut". That argument is spent; 1.0 should be re-justified on what it
  actually resets rather than inheriting this one.
- **`accountability` is still `scored: false`** and now carries a quoted `0.1`. If it is going to
  stay unscored it should say so where a reader meets it, not only in the rubric.
...
`upsert` (weight 0.10, conditional) scores create-or-update ergonomics from the OpenAPIs
...
facet gets it for free (roadmap#270).
...
Next in this family: nothing here yet reaches WRITE SAFETY more broadly — idempotency keys,
conditional requests (`If-Match`/ETag) and documented retry semantics are all measurable from the
same contracts and all sit in the same gap this facet opened. They belong together in a later
version rather than bolted onto this one.
