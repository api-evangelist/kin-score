...
score block now carries one name each.

| was | is | why |
|---|---|---|
| `governance` | `contract_governance` | It scores rulesets, vocabularies, conformance and overlays — **artifacts describing a contract**. It says nothing about how an organisation governs itself. |
| `commercial_clarity` | `access_clarity` | For a statutory interface or an open-data API there are no commercial terms, and that is a third of the catalog. The questions the facet asks — what does it cost, what may you do, how do you get in — still apply. |

0.12 shipped both spellings deliberately so consumers could migrate without a hard cut, and
recorded the intent to drop the old pair at 1.0, "the stable release that resets every `kin/score`
artifact anyway."

### Why it comes forward

Because the dual-emit turned out to be a live defect rather than a harmless kindness.
`/rating/facets` served **eight names for six facets**, measured on three providers:

```
access_clarity       76.3  ==  commercial_clarity   76.3
contract_governance  33.3  ==  governance           33.3
```

Anyone summing that object double-counted two of nine. The published `FacetName` enum listed
neither alias, so there was no way to learn it from the contract either. A migration aid that
silently corrupts a sum is not an aid, and the 1.0 boundary is far enough away that leaving it
meant leaving a wrong number reachable.

The 0.12 note also weighed the cost of a hard cut at ~27,300 files, "669 report bundle data files
across 118 **sold** reports" among them. That cost has largely expired: the reports were retired
on 2026-08-21 and now answer **410 Gone**, the 26,574 provider pages are regenerated by the scorer,
and the badge renderer turned out to be one file rather than 38. Measured, the real consumer count
was **15 files across 4 repos**.

### No score moves

Verified on three providers spanning the range, before and after:

```
stripe    composite 79.7 -> 79.7   band exemplar    facets identical under the rename
up-bank   composite 46.8 -> 46.8   band developing  facets identical under the rename
apis-io   composite 88.1 -> 88.1   band exemplar    facets identical under the rename
```

The keys changed; nothing measured changed. **No band re-cut**, and the weights still sum to 1.0.

### The old names still work as INPUT

`?sort=governance` behaves exactly as before. Dropping a value callers already send breaks them for
no benefit — the defect was the duplicated **output**, not the accepted input. The old spellings are
normalised at the point of use, so a request sorting by a retired key sorts correctly rather than
silently ordering every provider by `-1`.

Reading surfaces — the three glyph renderers, both provider layouts, the listing include — fall
back to the pre-0.21.0 key when a page has not been rescored yet, so nothing renders a zero in the
window between the scoring pass and the deploys that follow it.

### Also: the standalone layer versions are strings

`agent_readiness.schema_version` and `accountability.schema_version` were unquoted two-part numbers,
which YAML and JSON both read as **floats**:

```ruby
0.2 == 0.20                 # => true
YAML.load("v: 0.20")["v"]   # => 0.2
```

So the agent-readiness layer could never have reached a version `0.20` — it would have been silently
identical to the `0.2` it already carried. It was also served as a **number** on `/agent-readiness`
and a **string** on its sibling endpoints, which no typed client can model. Both are quoted now;
nothing compares them arithmetically.
